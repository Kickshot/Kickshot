...
public class SourcePlayer : MonoBehaviour
{
    ...
    private bool ChangeHeight(float newHeight)
    {
        ...
        if (diff > 0f)
        {
            if (RaycastForHeadroom(out hit, diff))
            { // if we hit our head, just quit.
                return false;
            }
        }
        if (diff == 0)
    ...
    private void CheckCrouched()
    {
        ...
        else
        {
            ...
            if (!ChangeHeight(newheight))
            { // If we fail to change heights, we don't progress the timer.
                crouchTimer += Time.deltaTime;
            }
        }
    ...
    private void Friction()
    {
        ...
        if (groundEntity != null && !Input.GetButton("Jump"))
    ...
    private void StayOnGround()
    {
        ...
        RaycastHit outhit;
        if (!RaycastForGround(out outhit))
        { // If we slid into the air, discard the move.
    ...
    private void AirMove()
    {
        ...
        if ((Mathf.Abs(command.z) != 0f && Mathf.Abs(command.x) != 0f) || flatvel.magnitude <= 1f)
        { // Trying to move diagonally, or is currently unaccelerated.
            Accelerate(wishdir, airAccelerate, flySpeed);
        }
        else if (Mathf.Abs(command.z) != 0f && Mathf.Abs(command.x) == 0f)
        { // Trying to move forward/backward.
            if (command.z == -1f && check < -0.9f)
        ...
        else if (Mathf.Abs(command.z) == 0f && Mathf.Abs(command.x) != 0f && Mathf.Abs(check) < 0.75f)
        { // Trying to air-strafe.
            ...
            if (fcheck > 0.01f && pvel.magnitude > 0.1f)
            { // Only give the speed bonus if we're moving, otherwise we oscillate like crazy.
                fcheck -= 0.01f;
                fcheck /= 0.99f;
                fcheck *= 4f;
                float speedFunc;
                // Use a disjointed function to check how much speed to gain.
                if (fcheck > 0 && fcheck <= 0.1f)
                { // Exponentially scales up
                    speedFunc = (fcheck * 10f) * (fcheck * 10f);
                }
                else
                { // Logarithmically scales down
        ...
        { // Strafing in the air, but without trying to air-strafe.
            if (check < -0.9f)
            { // Give an acceleration bonus based on if they're trying to stop.
                Accelerate(wishdir, airAccelerate * airBreak, flySpeed);
            }
